...
The folder XMLConformanceTestingWork contains source and test contents that I used to do the infoset support testing. eclipse project files are also provided so that it can be readily loaded as a java project in IDE to quickly rerun the test. (Jar files location might have to be changed).
...
Apart from these the testing code is under the 'org.apache.axis.om' package inside the project. The file XMLConformanceTest.java has the latest code I used to test the parsing and serialization stuff. The content of wiki is pretty outdated and this is the final one.

** The XMLSuite.zip provided here is a zip of a directory that contains only the valid version 1.0 compliant XML files
** For those who want to rerun the tests and see the results, please edit the location of the XMLSuite directory (line 50 of org.apache.axis.om.infoset.XMLConformanecTest under test directory) to the location where you would unzip this XMLSuite.zip given here.
